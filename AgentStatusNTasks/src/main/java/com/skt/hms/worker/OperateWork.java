    ...
    public int LoadMembers(String IpAddr) {
        ...
        try {
            ...
            if (StringUtils.isEmpty(IpAddr)) {
                sql = "SELECT member_seq,member_name,member_host,rest_api_port,agreement_seq FROM CM_MEMBER_INFO WHERE use_flag=1 ";
            }
            else {
                sql = "SELECT member_seq,member_name,member_host,rest_api_port,agreement_seq FROM CM_MEMBER_INFO WHERE use_flag=1 and member_host='"+IpAddr.trim()+"'";
    ...
    public void UpdateStatusTasks() {
        ...
        try {
            ...
            for (int i=0; i<taskList.size(); i++)  {
                ...
                sql = "INSERT INTO task_status(insert_time,agreement_seq,node_seq,host_name,ip,task_id,task_name,task_status,task_deadline,task_last_failure_message,alter_agreement_flag)" +
                        " VALUES (" +
                        "now()" +
                        ...
                        ",'" + task.getLast_failure_message() +"'" +    //last_failure_message
                        ", 1" +
                        ") ON DUPLICATE KEY UPDATE " +
                        "insert_time=now()" +
                        ...
                        ",task_last_failure_message='" + task.getLast_failure_message() +"'" +
                        ",alter_agreement_flag=1"
                ;
