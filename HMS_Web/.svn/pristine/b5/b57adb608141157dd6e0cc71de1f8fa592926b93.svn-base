                ...
                controller: ['$scope', '$templateCache', '$interpolate', 'treeConfig', function ($scope, $templateCache, $interpolate, treeConfig) {
                    ...
                    $scope.headClass = function(node) {
                        ...
                        if ($scope.options.isLeaf(node, $scope))
                            return "tree-leaf" + injectSelectionClass+ hideClass;
                        if ($scope.expandedNodesMap[this.$id])
                            return "tree-expanded" + injectSelectionClass + hideClass;
                        else
                            return "tree-collapsed" + injectSelectionClass + hideClass;
                    };
                    ...
                    if(!template) {
                        var index = $scope.index ? ("node." + $scope.index) : '$index';
                        template =
                            '<ul {{options.ulClass}} >' +
                            '<li ng-repeat="node in node.{{options.nodeChildren}} | filter:filterComparator {{options.orderBy}}" ng-class="headClass(node)" {{options.liClass}}' +
                            'set-node-to-data>' +
