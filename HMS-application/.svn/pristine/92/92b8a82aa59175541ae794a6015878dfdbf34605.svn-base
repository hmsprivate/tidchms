...
//						String query = String.format("select * from \"%s\" where source=\'%s\' order by time desc limit 1",	facter_measurement, node_info.get("member_name"));
						String query = String.format("select * from \"%s\" where source=\'%s\' and time > now() - 7d order by time desc limit 1",	facter_measurement, node_info.get("member_name"));
...
//					logger.debug("{}", new Gson().toJson(node_data));
				...
				if (node_info_data.isEmpty()) throw new Exception("No Data from influx !!!"); 
...
//									}
				...
				/*
				 *  node_detail_info_list size 너무 크면 분활해서 insert 
				 */
				List<List<Map<String, Object>>> splitLists = ListUtils.partition(node_detail_info_list, 10000);
				for (List<Map<String, Object>> split_insert_data : splitLists) {
					logger.debug("{} count insert to node_info_detail, node_info_history ", split_insert_data.size());
					
					Map<String, Object> insert_node_detail_info_map = new HashMap<>();
					insert_node_detail_info_map.put("node_detail_info_list", split_insert_data);
...
//		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'");
		...
		return t_sdf.format(milliseconds);
